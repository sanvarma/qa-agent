...
async function runGeneric(args: RunArgs): Promise<void> {
  ...
  const result = await runAgent(
    ...
    { llm, tools, toolCtx: { repoRoot, runDir, paths: cfg.paths, selectorPreference: cfg.browse?.selectorPreference, setupFlows: cfg.setupFlows }, state },
  );
...
async function runQa(args: QaArgs): Promise<void> {
  ...
  const result = await runOrchestrator(
    ...
    {
      llm,
      toolCtx: { repoRoot, runDir, paths: cfg.paths, selectorPreference: cfg.browse?.selectorPreference, setupFlows: cfg.setupFlows },
