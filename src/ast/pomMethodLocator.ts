...
export interface MethodLocator {
  className: string;
  name: string;
  startLine: number;
  endLine: number;
...
export type FindMethodResult =
  | { status: 'found'; locator: MethodLocator; methodNode: MethodDeclaration }
  | { status: 'not_found'; methodCandidates: string[] }
  | { status: 'is_accessor'; kind: 'getter' | 'setter' }
  | { status: 'is_constructor' }
  | { status: 'abstract' }
  | { status: 'overloaded'; implementationLine?: number; signatureLines: number[] };

export function findMethod(cls: ClassDeclaration, name: string): FindMethodResult {
  // Accessors are handled by the selector tool, not this one.
  const getter = cls.getGetAccessor(name);
  if (getter) return { status: 'is_accessor', kind: 'getter' };
  const setter = cls.getSetAccessor(name);
  if (setter) return { status: 'is_accessor', kind: 'setter' };

  // Constructor is reserved for a future tool with different safety semantics.
  if (name === 'constructor') return { status: 'is_constructor' };
