import { Node, CallExpression, ArrowFunction, FunctionExpression } from 'ts-morph';
...
export type EditErrorCode =
  | 'no_callback'
  | 'callback_not_a_function'
  | 'expression_bodied_arrow'
  | 'parameterized_test'
  | 'body_parse_failed';

export class EditError extends Error {
  constructor(public code: EditErrorCode, message: string) {
    super(message);
    this.name = 'EditError';
  }
}
 ...
 */
function getCallbackArg(call: CallExpression): Callback {
 ...
 */
export function replaceTestBody(call: CallExpression, newBody: string): void {
  ...
  if (Node.isArrowFunction(cb)) {
    ...
    if (!Node.isBlock(body)) {
      throw new EditError(
        'expression_bodied_arrow',
        'callback has an expression body; only block-bodied callbacks are supported',
      );
    }
  }
