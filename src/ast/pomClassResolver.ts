...
export type ResolveClassResult =
  | { status: 'found'; cls: ClassDeclaration }
  | { status: 'no_class' }
 ...
 */
export function resolveClass(sf: SourceFile, name: string | undefined): ResolveClassResult {
  const classes = sf.getClasses().filter((c) => !!c.getName());
  const names = classes.map((c) => c.getName() as string);

  if (name) {
    const match = classes.find((c) => c.getName() === name);
    if (match) return { status: 'found', cls: match };
    return { status: 'not_found', candidates: names };
  }

  if (classes.length === 0) return { status: 'no_class' };
  if (classes.length === 1) return { status: 'found', cls: classes[0] };
