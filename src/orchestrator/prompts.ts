...
export function regenerateSystemPrompt(locales: string[]): string {
  return `You are a QA automation agent in the REGENERATE phase.

A test was generated and then repaired ${String(0)} times, but it is still failing. Your job is to REWRITE the test from scratch using the original test case and the full failure history as context.

Available tools:
  - fs.read: read any file in the repo
  - test.editCase: replace the body of an existing test (preferred — the spec file already exists)
  - test.addCase: add a test if the spec file exists but the case was removed
  - test.createSpec: create a new spec file only if the existing one is unrecoverable
  - ast.addImport: add imports
  - pom.updateSelector: fix a POM selector
  - pom.editMethod: replace a POM method body

The following tools may also be available depending on configuration:
  - browse.navigate / browse.snapshot / browse.click / browse.type / browse.hover

## Framework structure

Page objects live in two tiers:
  - src/pages/common/<Page>.ts           — default for all locales
  - src/pages/locales/<locale>/<Page>.ts — locale-specific override

Configured locales: ${locales.join(', ')}

Fixtures — always import from src/fixtures/pages.fixture.ts, never from @playwright/test.

## Rules

  - Read the existing test first (fs.read the spec file) so you know what was written before.
  - Read the relevant POM(s) before writing the replacement.
  - The new test MUST exercise the assertion in 'Expected' — do not water it down.
  - If browse.* tools are available, use browse.navigate + browse.snapshot to confirm selectors
    against the live DOM before writing — do not guess selectors that previously failed.
  - Prefer test.editCase over deleting and recreating the spec.
  - Stop as soon as the rewrite is done. DO NOT run tests.`;
}
...
export function regenerateTask(
  ...
  const historyLines = history.map((h, i) =>
    [
      `  Attempt ${i + 1}: kind=${h.classification.kind} action=${h.classification.action}`,
      `    Error: ${h.failure.message.split('\n')[0]}`,
    ].join('\n'),
  );

  return [
    `Rewrite the following test. It has failed ${history.length} fix attempt(s) and needs a fresh approach.`,
    '',
    '## Original test case',
    renderTestCase(tc),
    '',
    ...
    '',
    '## Fix history (all failed — do NOT repeat these approaches)',
    ...historyLines,
    '',
    `## Configured locales: ${locales.join(', ')}`,
    '',
    'Steps:',
    '  1. fs.read the current spec file to see what was written.',
    '  2. fs.read the POM(s) the Expected outcome references (common/ first, then locale overrides).',
    '  3. If browse.* tools are available, use browse.navigate + browse.snapshot to verify',
    '     selectors against the live app before writing.',
    '  4. Use test.editCase to replace the test body with a correct implementation.',
    '     Do not repeat selectors or approaches from the fix history above.',
    '  5. If any POM selector or method is wrong, fix it first via pom.updateSelector / pom.editMethod.',
    '  6. Stop.',
  ].join('\n');
}
