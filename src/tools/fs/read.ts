...
export const fsReadTool: Tool<Input, Output> = {
  name: 'fs.read',
  description:
    'Read a UTF-8 text file from the target repo. Path must be relative to repo root. ' +
  ...
  async run(input, ctx) {
    ...
    if (start > end) throw new Error('startLine > endLine');
