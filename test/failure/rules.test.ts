...
import type { NormalizedFailure } from '../../src/tools/exec/parseReport.js';
...
describe('failure.classify — unknown fallback', () => {
  ...
  test('unknown fallback with useLlmFallback=true but no provider returns unknown', async () => {
    const c = await classifyFailure(
      failure({ message: 'totally novel error', rawSnippet: '' }),
      { useLlmFallback: true },
    );
    assert.equal(c.kind, 'unknown');
  ...
  test('LLM fallback error is caught and reported in reasoning', async () => {
    const c = await classifyFailure(
      failure({ message: 'novel', rawSnippet: '' }),
      {
        useLlmFallback: true,
        llm: {
          async classify() {
