...
describe('ast.addImport — success', () => {
  ...
  test('adds named import to a file under paths.tests', async () => {
    ...
    const out = await astAddImportTool.run(
      { file: 'tests/x.spec.ts', from: '@playwright/test', named: ['test'], typeOnly: false },
      repo.toolCtx,
    );
  ...
  test('adds named import to a file under paths.pages (dual-scope)', async () => {
    ...
    const out = await astAddImportTool.run(
      { file: 'src/pages/P.ts', from: '@playwright/test', named: ['Page'], typeOnly: false },
      repo.toolCtx,
    );
...
describe('ast.addImport — refusals', () => {
  ...
  test('refuses paths outside both paths.tests and paths.pages', async () => {
    ...
    await assert.rejects(
      () =>
        astAddImportTool.run(
          { file: 'lib/util.ts', from: '@playwright/test', named: ['test'], typeOnly: false },
          repo.toolCtx,
        ),
      /scope violation/,
    );
  ...
  test('errors on conflicting default import', async () => {
    ...
    await assert.rejects(
      () =>
        astAddImportTool.run(
          { file: 'tests/x.spec.ts', from: 'lodash', default: '_', typeOnly: false },
          repo.toolCtx,
        ),
      /conflicting_default/,
    );
