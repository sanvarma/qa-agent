...
describe('resolveWithinScope — refusals', () => {
  test('refuses absolute input path', () => {
    assert.throws(
      () => resolveWithinScope(resolve('/etc/passwd'), 'tests', ctx),
  ...
  test('refuses path that escapes repo root with ..', () => {
    assert.throws(
      () => resolveWithinScope(`..${sep}outside.ts`, 'tests', ctx),
      /escapes repo root/,
    );
  });
  ...
  test('refuses tests path when scope is pages', () => {
    assert.throws(
      () => resolveWithinScope('tests/login.spec.ts', 'pages', ctx),
      /scope violation.*pages/,
    );
  });

  test('throws when ToolContext.paths is missing', () => {
    const noPathCtx: ToolContext = { repoRoot };
    assert.throws(
      () => resolveWithinScope('tests/x.ts', 'tests', noPathCtx),
      /paths is required/,
    );
  });
});
