...
function makeReport(overrides: Partial<{ stats: unknown; suites: unknown[] }> = {}): string {
  const base = {
...
describe('exec.parseReport — valid report', () => {
  ...
  test('reports zero failures on an empty report', async () => {
    ...
    const out = await parseReportTool.run(
      { reportPath, maxFailures: 25 },
      repo.toolCtx,
    );
...
describe('exec.parseReport — error cases', () => {
  ...
  test('throws clear error when report file is missing', async () => {
    ...
    await assert.rejects(
      () => parseReportTool.run({ reportPath, maxFailures: 25 }, repo.toolCtx),
      /report not found/,
    );
  });
  ...
  test('throws when reportPath is relative', async () => {
    ...
    await assert.rejects(
      () =>
        parseReportTool.run(
          { reportPath: 'relative/path.json', maxFailures: 25 },
          repo.toolCtx,
        ),
      /must be absolute/,
    );
