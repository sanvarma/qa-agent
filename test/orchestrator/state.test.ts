...
import { initialState, transition } from '../../src/orchestrator/state.js';

describe('initialState', () => {
  test('creates a state in phase "init" with zero fix attempts', () => {
    const s = initialState('run-123');
    assert.equal(s.phase, 'init');
    assert.equal(s.fixAttemptsUsed, 0);
    assert.equal(s.runId, 'run-123');
    assert.equal(s.attempts.length, 0);
    assert.equal(s.fixHistory.length, 0);
