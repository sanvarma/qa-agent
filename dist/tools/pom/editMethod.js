...
export const pomEditMethodTool = {
    name: 'pom.editMethod',
    description: 'Replace the body of a named instance method on a page-object class. ' +
    ...
    async run(input, ctx) {
        ...
        switch (find.status) {
            case 'not_found':
                throw new Error(`method '${input.name}' not found on class '${cls.getName()}'. ` +
                    `Methods in class: ${find.methodCandidates.join(', ') || '(none)'}`);
            case 'is_accessor':
                throw new Error(`'${input.name}' is a ${find.kind}, not a method. ` +
            ...
            case 'overloaded':
                throw new Error(`'${input.name}' is overloaded (signatures at lines ${find.signatureLines.join(', ')}, ` +
...
//# sourceMappingURL=editMethod.js.map