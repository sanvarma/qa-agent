...
import { resolveClass } from '../../ast/pomCommon.js';
...
export const pomUpdateSelectorTool = {
    name: 'pom.updateSelector',
    description: 'Update the selector string of a field-initializer selector on a page-object class. ' +
    ...
    async run(input, ctx) {
        ...
        if (clsResult.status === 'not_found') {
            throw new Error(`class '${input.class}' not found in ${input.file}. Classes in file: ${clsResult.candidates.join(', ') || '(none)'}`);
        }
        if (clsResult.status === 'ambiguous') {
            throw new Error(`multiple classes in ${input.file}: ${clsResult.candidates.join(', ')}. Pass 'class' to disambiguate.`);
        }
        const cls = clsResult.cls;
        const find = findSelectorField(cls, input.name);
        switch (find.status) {
            case 'not_found':
                throw new Error(`field '${input.name}' not found on class '${cls.getName()}'. ` +
                    `Fields in class: ${find.fieldCandidates.join(', ') || '(none)'}`);
            case 'not_a_selector_field':
                throw new Error(`'${input.name}' is not a selector field: ${find.reason}. ` +
                    `Expected shape: \`${input.name} = this.page.locator('...')\`.`);
            case 'chained_selectors':
                throw new Error(`'${input.name}' uses chained locators (e.g. .locator(...).locator(...)). ` +
                    `Refused: chained selectors require a broader edit than this tool supports.`);
            case 'role_based_selector':
                throw new Error(`'${input.name}' uses getByRole, whose first arg is a role, not a selector. ` +
                    `Refused: use pom.editMethod to change role-based lookups.`);
            case 'unsupported_call':
                throw new Error(`'${input.name}' uses unsupported locator call '${find.callName}'. ` +
                    `Supported: locator, getByTestId, getByText, getByLabel, getByPlaceholder, getByAltText, getByTitle.`);
            case 'non_string_selector':
                throw new Error(`'${input.name}' does not have a string literal as its first argument. ` +
...
//# sourceMappingURL=updateSelector.js.map