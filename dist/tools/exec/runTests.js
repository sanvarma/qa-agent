...
export function createRunTestsTool(defaults) {
    return {
        name: 'exec.runTests',
        description: 'Run the configured Playwright test command and capture the JSON report. ' +
        ...
        async run(input, ctx) {
            ...
            if (input.grep) {
                ...
                if (input.grep.includes("'"))
                    throw new Error("grep must not contain single quotes");
                parts.push(`--grep '${input.grep}'`);
            }
            if (input.extraArgs && input.extraArgs.length > 0) {
                for (const a of input.extraArgs) {
                    if (a.includes("'"))
                        throw new Error('extraArgs must not contain single quotes');
                    parts.push(`'${a}'`);
                }
            }
