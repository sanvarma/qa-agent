...
describe('test.createSpec — success', () => {
    ...
    test('creates a minimal spec file with the Playwright import', async () => {
        ...
        const out = await testCreateSpecTool.run({ file: 'tests/new.spec.ts' }, repo.toolCtx);
    ...
    test('includes a describe block when requested', async () => {
        repo = await makeTempRepo();
        await testCreateSpecTool.run({ file: 'tests/auth.spec.ts', describe: 'Auth' }, repo.toolCtx);
...
describe('test.createSpec — refusals', () => {
    ...
    test('refuses to overwrite an existing file', async () => {
        ...
        await assert.rejects(() => testCreateSpecTool.run({ file: 'tests/existing.spec.ts' }, repo.toolCtx), /already exists/);
    ...
    test('refuses paths outside paths.tests', async () => {
        repo = await makeTempRepo();
        await assert.rejects(() => testCreateSpecTool.run({ file: 'src/pages/wrong-place.ts' }, repo.toolCtx), /scope violation.*tests/);
    });
});
//# sourceMappingURL=createSpec.test.js.map