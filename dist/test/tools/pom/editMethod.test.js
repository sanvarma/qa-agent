...
describe('pom.editMethod — success', () => {
    ...
    test('replaces method body and preserves signature', async () => {
        ...
        const pomPath = await repo.write('src/pages/LoginPage.ts', [
...
describe('pom.editMethod — refusals', () => {
    ...
    test('refuses to edit a getter (redirects to pom.updateSelector)', async () => {
        repo = await makeTempRepo();
        await repo.write('src/pages/P.ts', [
            'export class P {',
            '  get field() { return 1; }',
            '}',
            '',
        ].join('\n'));
        await assert.rejects(() => pomEditMethodTool.run({
            file: 'src/pages/P.ts',
            name: 'field',
            newBody: 'return 2;',
        }, repo.toolCtx), /getter.*pom\.updateSelector/);
    });
    ...
    test('surfaces body_parse_failed on syntactically invalid newBody', async () => {
        ...
        }, repo.toolCtx), /body_parse_failed/);
    });
});
//# sourceMappingURL=editMethod.test.js.map