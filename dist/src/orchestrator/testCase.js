...
export async function loadTestCase(path) {
    ...
    try {
        ...
        throw new Error(`test case file is not valid JSON: ${err.message}`);
    }
 ...
 */
export function renderTestCase(tc) {
    const lines = [];
    lines.push(`Title: ${tc.title}`);
    if (tc.describe)
        lines.push(`Describe: ${tc.describe}`);
    if (tc.specFile)
        lines.push(`Spec file: ${tc.specFile}`);
    lines.push('Steps:');
